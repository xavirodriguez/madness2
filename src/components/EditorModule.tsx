...
export const EditorModule: React.FC<EditorModuleProps> = ({
  ...
  const [nameDisplay, setNameDisplay] = useState(
    editingAvatar?.name_display || ""
  );
  const [rarity, setRarity] = useState<AvatarRarity>(
    editingAvatar?.rarity || "RARE"
  );
  const [status, setStatus] = useState<AvatarStatus>(
    editingAvatar?.status || "DRAFT"
  );
  ...
  const [description, setDescription] = useState(
    editingAvatar?.description || ""
  );
  const [tagsInput, setTagsInput] = useState(
    editingAvatar?.tags?.join(", ") || "LiveOps, Season4"
  );
  ...
  useEffect(() => {
    if (editingAvatar) {
      ...
      setDescription(
        "New avatar under calibration for the upcoming tournament season."
      );
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const updated: Avatar = {
      id: editingAvatar
        ? editingAvatar.id
        : `av_${slug.toLowerCase().replace(/[^a-z0-9]/g, "_")}`,
      slug: slug.trim(),
      ...
      created_at: editingAvatar
        ? editingAvatar.created_at
        : new Date().toISOString().replace("T", " ").substring(0, 16) + " UTC",
      tags: tagsInput
        .split(",")
        .map((t) => t.trim())
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 sm:gap-6">
            ...
            >
              ...
              {uploadProgress !== null && (
                <div className="absolute inset-0 bg-[#07011E]/90 backdrop-blur-md flex flex-col items-center justify-center p-6 space-y-3 z-20">
                  ...
                  <span className="text-[11px] font-mono-code text-[#00E5FF]">
                    {uploadProgress}%
                  </span>
                </div>
              )}
            ...
            >
              ...
              >
                ...
                <div className="text-xs font-bold text-white">
                  Drag asset here or{" "}
                  <span className="text-[#00E5FF] underline">
                    click to browse
                  </span>
                </div>
                <p className="text-[10px] text-purple-300/60">
                  24-bit PNG with transparency or WebP (1024x1024px
                  recommended). Simulates direct push to S3 bucket.
                </p>
              </label>
            </div>
        ...
        <div className="lg:col-span-7 space-y-5">
          <form
            ...
            className="bg-[#16083D] rounded-2xl border border-[#2E146D] p-6 space-y-5 shadow-[0_8px_30px_rgba(5,0,20,0.6)]"
          >
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <span className="text-[10px] text-purple-300/60 font-mono-code">
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                {isKeySuffixValid && nameKeySuffix.trim() ? (
                  ...
                  <span className="text-[10px] text-amber-400 flex items-center gap-1 font-semibold">
                    <AlertTriangle className="w-3 h-3" /> Uppercase, numbers,
                    and '_' only
                  </span>
                )}
              </div>
              ...
              <div className="flex items-center rounded-xl bg-[#07011E] border border-[#2E146D] focus-within:border-[#00E5FF] focus-within:ring-1 focus-within:ring-[#00E5FF] overflow-hidden">
                ...
                <input
                  ...
                  value={nameKeySuffix}
                  onChange={(e) =>
                    setNameKeySuffix(e.target.value.toUpperCase())
                  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  >
                    <option value="COMMON" className="bg-[#07011E]">
                      COMMON (Slate-500)
                    </option>
                    <option value="RARE" className="bg-[#07011E]">
                      RARE (Blue-600)
                    </option>
                    <option value="EPIC" className="bg-[#07011E]">
                      EPIC (Purple-600)
                    </option>
                    <option value="LEGENDARY" className="bg-[#07011E]">
                      ★ LEGENDARY ★ (Amber-500/Gold)
                    </option>
                  </select>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-[#050014]/85 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="bg-[#1A0948] border-2 border-rose-500/80 rounded-2xl max-w-lg w-full max-h-[90vh] overflow-y-auto p-5 sm:p-6 shadow-[0_0_40px_rgba(244,63,94,0.35)] space-y-5 animate-in fade-in zoom-in-95 duration-150">
            <div className="flex items-start gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-rose-300 font-bold leading-relaxed">
                  "Cannot archive an avatar with active LiveOps rules.
                  Deactivate linked rules first."
                </p>
              </div>
            </div>
                ...
                {offendingRules.map((rule) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[11px] text-purple-300/70 truncate max-w-[260px]">
                        {rule.name}
                      </div>
                    </div>
            ...
            <p className="text-xs text-purple-300/80 leading-relaxed">
              Archiving this avatar would cause execution errors or probability
              overflow if players fulfill spin requirements in mobile clients.
            </p>
